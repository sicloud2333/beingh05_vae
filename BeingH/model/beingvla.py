...
class BeingHConfig(PretrainedConfig):
    ...
    def __init__(
            ...
            wrist_action_loss_weight=1.0,
            wrist_action_dim=6,
            temporal_delta_loss_weight=0.0,
            temporal_delta_huber_beta=1.0,
        ...
        self.wrist_action_loss_weight = float(wrist_action_loss_weight)
        if self.wrist_action_loss_weight <= 0:
            raise ValueError("wrist_action_loss_weight must be positive")
        self.wrist_action_dim = int(wrist_action_dim)
        if self.wrist_action_dim <= 0:
            raise ValueError("wrist_action_dim must be positive")
        self.temporal_delta_loss_weight = float(temporal_delta_loss_weight)
        if self.temporal_delta_loss_weight < 0:
            raise ValueError("temporal_delta_loss_weight must be non-negative")
        self.temporal_delta_huber_beta = float(temporal_delta_huber_beta)
        if self.temporal_delta_huber_beta <= 0:
            raise ValueError("temporal_delta_huber_beta must be positive")
...
class BeingH(PreTrainedModel):
    ...
    def __init__(self, language_model, vit_model, connector, config: BeingHConfig, use_flash_attn=True):
        ...
        # Training-only loss scaling. It creates no parameters/buffers and is not
        # consulted by get_action(), preserving inference for existing checkpoints.
        self.wrist_action_loss_weight = float(
            getattr(config, "wrist_action_loss_weight", 1.0)
        )
        # Old checkpoints do not record wrist_action_dim and retain 0:6.
        self.wrist_action_dim = int(getattr(config, "wrist_action_dim", 6))
        if self.wrist_action_dim <= 0:
            raise ValueError("wrist_action_dim must be positive")
        self.temporal_delta_loss_weight = float(
            getattr(config, "temporal_delta_loss_weight", 0.0)
        )
        self.temporal_delta_huber_beta = float(
            getattr(config, "temporal_delta_huber_beta", 1.0)
        )
        if self.temporal_delta_loss_weight > 0 and not self.use_flow_matching:
            raise ValueError(
                "temporal_delta_loss_weight is only supported with flow matching"
            )
    ...
    def _apply_wrist_action_loss_weight(
        self,
        loss_mask: torch.Tensor,
    ) -> torch.Tensor:
        """Weight configured unified wrist slots without changing loss scale."""
        if self.wrist_action_loss_weight == 1.0:
            return loss_mask
    ...
    def forward(
        ...
        **kwargs
        ...
        if padded_state is not None:
            ...
            if self.use_flow_matching:
                ...
                if (self.config.use_training_time_rtc and
                    ...
                    noisy_trajectory_3d = noisy_trajectory.view(
                        B, self.action_chunk_length, -1
                    )
                    reconstruction_t_3d = t_full.view(
                        B, self.action_chunk_length, 1
                    )
                    ...
                    velocity_target = padded_action_target - noise
                    reconstruction_t_3d = t_reshaped
        ...
        flow_action_loss = torch.tensor(0.0, device=device)
        temporal_delta_loss = torch.tensor(0.0, device=device)
        if padded_action is not None:
            ...
            if last_hidden_state_act.shape[0] > 0:
                ...
                if self.use_flow_matching:
                    ...
                    elementwise_action_loss = F.mse_loss(
                        predicted_velocity.view(B * self.action_chunk_length, -1),
                        velocity_target,
                        reduction="none",
                    )
                    token_loss_mask = padded_action_mask.float()
                    ...
                    if (self.config.use_training_time_rtc and
                        ...
                        postfix_mask = ~prefix_mask_rtc.view(-1, 1).expand_as(
                            padded_action_mask
                        )
                        token_loss_mask = token_loss_mask * postfix_mask.float()

                    # Weight the configured wrist span (0:6 for legacy
                    # Euler/axis-angle; 0:9 for Rot6D). The weighted denominator
                    # keeps loss magnitude comparable to
                    # the legacy weight=1 behavior.
                    weighted_token_loss_mask = (
                        self._apply_wrist_action_loss_weight(token_loss_mask)
                    )
                    flow_action_loss = (
                        elementwise_action_loss * weighted_token_loss_mask
                    ).sum() / (weighted_token_loss_mask.sum() + 1e-8)
                    action_loss = flow_action_loss

                    # Reconstruct the clean endpoint x_1 from the sampled flow
                    # state x_t and predicted velocity:
                    #   x_1_hat = x_t + (1 - t) * v_hat
                    # Then match the within-chunk first differences to the GT
                    # differences. This suppresses unsupported step-to-step
                    # spikes without penalizing genuine motion in the target.
                    if (
                        self.temporal_delta_loss_weight > 0
                        and self.action_chunk_length > 1
                    ):
                        clean_action_pred = (
                            noisy_trajectory_3d
                            + (1.0 - reconstruction_t_3d) * predicted_velocity
                        )
                        clean_action_target = padded_action_target.view(
                            B, self.action_chunk_length, -1
                        )
                        predicted_delta = (
                            clean_action_pred[:, 1:] - clean_action_pred[:, :-1]
                        )
                        target_delta = (
                            clean_action_target[:, 1:]
                            - clean_action_target[:, :-1]
                        )

                        # A temporal edge is valid only when both endpoint
                        # actions participate in the base flow loss. For RTC,
                        # this also prevents prefix-only/boundary edges from
                        # leaking into the postfix-only objective.
                        token_loss_mask_3d = token_loss_mask.view(
                            B, self.action_chunk_length, -1
                        )
                        temporal_loss_mask = (
                            token_loss_mask_3d[:, 1:]
                            * token_loss_mask_3d[:, :-1]
                        )
                        temporal_loss_mask = (
                            self._apply_wrist_action_loss_weight(
                                temporal_loss_mask
                            )
                        )
                        elementwise_temporal_delta_loss = F.smooth_l1_loss(
                            predicted_delta.float(),
                            target_delta.float(),
                            reduction="none",
                            beta=self.temporal_delta_huber_beta,
                        )
                        temporal_delta_loss = (
                            elementwise_temporal_delta_loss * temporal_loss_mask
                        ).sum() / (temporal_loss_mask.sum() + 1e-8)
                        action_loss = (
                            flow_action_loss
                            + self.temporal_delta_loss_weight
                            * temporal_delta_loss
                        )
                else:
                    ...
                    elementwise_action_loss = F.l1_loss(
                        predicted_actions.view(B * self.action_chunk_length, -1),
                        padded_action,
                        reduction="none",
                    )
                    loss_mask = self._apply_wrist_action_loss_weight(
                        padded_action_mask.float()
                    )
                    action_loss = (
                        elementwise_action_loss * loss_mask
