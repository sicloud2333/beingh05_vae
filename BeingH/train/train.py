...
class ModelArguments:
    ...
    wrist_action_loss_weight: float = field(
        default=1.0,
        metadata={
            'help': (
                'Training-only loss weight for unified wrist action slots 0:6. '
                'A value of 1.0 preserves legacy behavior.'
            )
        },
    )
    wrist_action_dim: int = field(
        default=6,
        metadata={
            'help': (
                'Number of leading unified action slots treated as wrist for '
                'training-only loss weighting. Legacy Euler/axis-angle uses 6; '
                'Rot6D uses 9.'
            )
        },
    )
    temporal_delta_loss_weight: float = field(
        default=0.0,
        metadata={
            'help': (
                'Weight of the Huber loss between adjacent clean-action '
                'increments inside each flow-matching action chunk. '
                'A value of 0.0 preserves legacy training behavior.'
            )
        },
    )
    temporal_delta_huber_beta: float = field(
        default=1.0,
        metadata={
            'help': (
                'Huber transition point (SmoothL1 beta) for the temporal '
                'clean-action delta loss.'
            )
        },
    )
...
class TrainingArguments(TrainingArguments):
    ...
    fused_optimizer: bool = field(
        default=False,
        metadata={"help": "Use the fused CUDA AdamW implementation."}
    )
...
def main():
    ...
    config = BeingHConfig(
        ...
        wrist_action_loss_weight=model_args.wrist_action_loss_weight,
        wrist_action_dim=model_args.wrist_action_dim,
        temporal_delta_loss_weight=model_args.temporal_delta_loss_weight,
        temporal_delta_huber_beta=model_args.temporal_delta_huber_beta,
    ...
    # Persist the exact data transform in config.json so inference cannot
    # accidentally apply q99/min-max/raw preprocessing from another run.
    data_config_names = list(
        dict.fromkeys(
            name
            for group in dataset_meta.values()
            if isinstance(group, dict)
            for name in group.get("data_config_names", [])
        )
    )
    if data_config_names:
        model.config.data_config_names = data_config_names
        if len(data_config_names) == 1:
            model.config.data_config_name = data_config_names[0]
        logger.info(
            f"Checkpoint data config(s): {data_config_names}"
        )
    ...
    optimizer = torch.optim.AdamW(
        ...
        fused=training_args.fused_optimizer,
    )
    logger.info(
        f"Optimizer: AdamW(fused={training_args.fused_optimizer}), "
        f"trainable parameter tensors={len(optimizer_parameters)}"
    )
    ...
    window_start_time = loop_start_time
    ...
    for micro_step, data in enumerate(train_loader, start=start_micro_step):
        ...
        with torch.amp.autocast(device.type, enabled=True, dtype=torch.bfloat16):
            ...
            temporal_delta_loss = loss_dict.get(
                'temporal_delta_loss',
                torch.zeros_like(action_loss),
            )
        ...
        accum_flow_action_loss += (
            flow_action_loss.detach()
            / training_args.gradient_accumulation_steps
        )
        accum_temporal_delta_loss += (
        ...
        if (micro_step + 1) % training_args.gradient_accumulation_steps == 0:
            ...
            if curr_step>0 and curr_step % training_args.logging_steps==0:
                ...
                window_elapsed = log_time - window_start_time
                avg_steps_per_sec = curr_step / elapsed_time_total if elapsed_time_total > 0 else 0
                window_steps_per_sec = (
                    window_optimizer_steps / window_elapsed
                    if window_elapsed > 0 else 0
                )
                ...
                global_window_samples = window_stats[0].item()
                global_window_tokens = window_stats[1].item()
                samples_per_sec = (
                    global_window_samples / window_elapsed
                    if window_elapsed > 0 else 0
                )
                tokens_per_sec = (
                    global_window_tokens / window_elapsed
                    if window_elapsed > 0 else 0
                )
                effective_global_batch = (
                    global_window_samples / window_optimizer_steps
                    if window_optimizer_steps > 0 else 0
                )
                ...
                avg_flow_act_loss = torch.tensor(
                    accum_flow_action_loss / training_args.logging_steps,
                    device=device,
                )
                dist.all_reduce(avg_flow_act_loss, op=dist.ReduceOp.SUM)
                avg_flow_act_loss = avg_flow_act_loss.item() / world_size

                avg_temporal_delta_loss = torch.tensor(
                    accum_temporal_delta_loss / training_args.logging_steps,
                    device=device,
                )
                dist.all_reduce(
                    avg_temporal_delta_loss,
                    op=dist.ReduceOp.SUM,
                )
                avg_temporal_delta_loss = (
                    avg_temporal_delta_loss.item() / world_size
                )
                ...
                message = (
                        ...
                        f"flow_loss: {avg_flow_act_loss:.4f} | "
                        f"delta_loss: {avg_temporal_delta_loss:.4f} | "
                        f"und_loss: {avg_und_loss:.4f} | "
                        f"lr: {scheduler.get_last_lr()[0]:.2e} | "
                        f"steps/s: {window_steps_per_sec:.2f} | "
                        f"samples/s: {samples_per_sec:.1f} | "
                        f"tokens/s: {tokens_per_sec:.0f} | "
                        f"global_bsz: {effective_global_batch:.1f} | "
                        f"ETA: {eta_str} | "
                ...
                accum_action_loss = 0.
                accum_flow_action_loss = 0.
                accum_temporal_delta_loss = 0.
                accum_ce_loss = 0.
                window_start_time = log_time
                window_optimizer_steps = 0
                window_local_samples = 0
                window_local_tokens = 0
